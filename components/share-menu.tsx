...
export function ShareMenu({
  ...
  const actionClass =
    "flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm text-slate-700 transition hover:bg-slate-50";
  ...
  return (
    <div ref={wrapperRef} className="relative">
      ...
      {menuOpen ? (
        <div className="absolute right-0 z-20 mt-2 w-72 rounded-2xl border border-slate-200 bg-white p-2 shadow-[0_16px_40px_rgba(15,23,42,0.14)]">
          <p className="px-3 pb-2 text-[11px] font-semibold uppercase tracking-[0.08em] text-slate-500">
            Share this
          </p>
      ...
      {repostOpen ? (
        ...
        >
          <div className="w-full max-w-lg rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_24px_60px_rgba(15,23,42,0.18)]">
            <div className="mb-4 flex items-center justify-between gap-3">
              <h3 className="text-base font-semibold text-slate-900">Repost to your feed</h3>
              <button
                type="button"
                onClick={() => setRepostOpen(false)}
                className="inline-flex h-9 items-center justify-center rounded-full border border-slate-300 px-4 text-sm font-medium text-slate-700 transition hover:bg-slate-50"
              >
                Close
              </button>
            </div>

            <form onSubmit={submitRepost} className="space-y-4">
              <textarea
                value={caption}
